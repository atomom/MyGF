    ...
    public static class VSCode
    {
        ...
        static void CheckForUpdate()
        {
            ...
            try
            {
                ...
                {
                    fileContent = webClient.DownloadString(@"
        ...
        static bool OnOpenedAsset(int instanceID, int line)
        {
            ...
            object[] args1 = OnOpenAsset2(instanceID, line);
            if (args1 != null)
            {
                string args = "\"" + ProjectPath + "\" -g \"" + args1[0] + ":" + args1[1].ToString() + "\" -r";

                // CallVSCode(args);
                InternalEditorUtility.OpenFileAtLineExternal(Utility.Path.GetCombinePath(Application.dataPath, args1[0].ToString()), (int) args1[1]);
                return true;
            }
            ...
            {
                string filepath = AssetDatabase.GetAssetPath(selected);
        ...
        private static readonly Regex LogRegex = new Regex(@"at.+Assets.(.+\.cs)\:(\d+)");
        private static object[] OnOpenAsset2(int instanceId, int line)
        {
            ...
            if (!s.Contains("/DefaultLogHelper")) { }
            else
            {
                // 跳过第一次匹配的堆栈
            ...
            string filename = s;
            int line1 = int.Parse(match.Groups[2].Value);

            return new object[] { filename, line1 };
        }
